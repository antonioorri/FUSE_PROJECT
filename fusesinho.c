...
{ // Devuelve el fichero de la ruta
	...
	{									// Mientras la longitud de la ruta sea distinta de 0
		index = strchr(path_name, '/'); // Busco /
		printf("%s---\n", index);

		if (index != NULL)
		{ // Si lo encuentra es un directorio
		...
		{									// Es fichero
			...
			{ // Por cada hijo del nodo actual (primera iteracion es root)
				if (strcmp((curr_node->children)[i]->name, curr_folder) == 0)
				{										  // Comparo si coincide con el nombre del fichero
					curr_node = (curr_node->children)[i]; // Asgino el nodo actual al hijo que coincide
...
{ // Para crear carpetas
	...
	*rindex = '\0';
	...
	new_folder->permissions = S_IFDIR | 755;
	new_folder->size = 0;
...
int myrename(const char *from, const char *to)
{
	...
	filetype *file = filetype_from_path(pathname); // Devuelve el fichero desde la ruta
...
int mywrite(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	filetype *file = filetype_from_path(pathname); // Busco el directorio 
